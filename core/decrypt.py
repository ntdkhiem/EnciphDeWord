...
def handler():
    while True:
            ...
            if cmd_encrypt == 'help' or cmd_encrypt == 'h' or cmd_encrypt == '?':
              ...
              if __platform__.startswith('Linux') and __platform__.startswith('Darwin'):
...
def loop_handler_word(function):
    ...
    key = ''
    while True:
        comd = raw_input("{0}{1}(Decrypt/{2}{3}{4}){5} >> ".format(BUNDERLINE,BBLUE,CRED,'The Vigenere Square',CEND,BEND)).strip()
        if comd == 'help' or comd == 'h' or comd == '?':
            help_ = helper.help_('decrypt','in')
            print help_.format(CLIGHTBLUE,CEND)
        elif comd == 'profile':
            ...
            try:
              key = raw_input("{}What string or word did you use as key:{} ".format(CGREEN,CEND)).strip()
              if key.isdigit():
                raise ValueError()
              print BOKMSG + " Changing encrypted key to >>{} {}\n".format(BEND,key)
            except ValueError:
